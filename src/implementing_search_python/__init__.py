...
quiet: Annotated[bool, typer.Option(help="Do not print matches")] = False,
):
    naive_search(references=reference, reads=reads, num_reads=num_reads, quiet=quiet)
...
quiet: Annotated[bool, typer.Option(help="Do not print matches")] = False,
    ...
    suffix_array_search(references=reference, reads=reads, num_reads=num_reads, quiet=quiet)
...
def fm_index_search_cmd(
    ...
    if use_pigeon:
        if not reference:
            ...
            return
        fm_index_pigeon_search(index_path=index, reads_path=reads, mismatches=mismatches, num_reads=num_reads, reference_path=reference, quiet=quiet)
    else:
        fm_index_search(index_path=index, reads_path=reads, mismatches=mismatches, num_reads=num_reads, quiet=quiet)
